...
class MSE_dir_Loss(nn.Module):
    def __init__(self, direction_lambda = 0.1):
        super(MSE_dir_Loss, self).__init__()
        self.direction_lambda = direction_lambda

    def forward(self, pred, target):
        mse_loss = nn.MSELoss(reduction='none')(pred, target)
        direction_loss = (torch.tanh(10*pred) - torch.tanh(10*target)) ** 2
        loss = mse_loss + self.direction_lambda * direction_loss
        return loss
...
#         covar = torch.mean((y_pred - mean_pred) * (y_true - mean_true), dim=0)
